...
def train(rank, a, h):
    ...
    if cp_g is None or cp_do is None:
        state_dict_g = load_checkpoint(
            os.environ.get("BIGVGAN_INIT_CKPT", "exp/bigvgan_init/g_00250000"), device
        )

        old_weight = state_dict_g["generator"]["conv_pre.weight_v"]
        new_weight = torch.zeros(old_weight.shape[0], 1473, old_weight.shape[2])
        new_weight = old_weight
        torch.nn.init.normal_(new_weight, mean=0.0, std=0.02)
        state_dict_g["generator"]["conv_pre.weight_v"] = new_weight
        ...
        # old_weight = state_dict_g["generator"]["conv_pre.weight_v"]
        # new_weight = torch.zeros(old_weight.shape[0], 1473, old_weight.shape[2])
        # new_weight[:, :1280, :] = old_weight
        # torch.nn.init.normal_(new_weight[:, 1280:, :], mean=0.0, std=0.02)
        # state_dict_g["generator"]['conv_pre.weight_v'] = new_weight
    ...
    """
    training_filelist, validation_filelist, list_unseen_validation_filelist = get_dataset_filelist(
        a
    )
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            y_g_hat = generator(x)[..., 32:-32]
            # print("YYYYYYYYYYYYYYYYY", h.sampling_rate, h.hop_size, y_g_hat.shape)
            y_g_hat_mel = mel_spectrogram(
                y_g_hat.squeeze(1),
                h.n_fft,
                80,
            ...
            clip_grad_norm = h.get("clip_grad_norm", 100.0)  # Default to 1000
            ...
            # print("XXXXXXXXXXX", y_mel.shape, y_g_hat_mel.shape)
