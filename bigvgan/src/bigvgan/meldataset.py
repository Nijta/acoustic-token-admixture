...
def upsample_time_axis(arr):
    x, d = arr.shape
    new_x = int(160 / 256 * x)

    # Create the new upsampled array
    upsampled_arr = np.zeros((new_x, d))

    # Create an array of the old indices and new indices
    old_indices = np.arange(x)
    new_indices = np.linspace(0, x - 1, new_x)

    for i in range(d):
        upsampled_arr[:, i] = np.interp(new_indices, old_indices, arr[:, i])

    return upsampled_arr


# import sys
# sys.path.insert(0, "/home/ubuntu/projects/FastSpeech2")
# from model import QuantizedWhisperForConditionalGeneration
# from data.utils import get_as_alignment
...
class MelDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        trained_model = torch.load(
            os.path.join(
                os.environ.get("RVQWHISPER_DIR", "rvqwhisper"),
                "exp/whisper_largev2_rvq8_en",
                "peft_model.pth",
            )
        )
        self.codebooks = [
            trained_model["state_dict"][
                f"base_model.model.model.vector_quantization.layers.{i}._codebook.embed"
            ][0, ...]
            .detach()
            .cpu()
            .numpy()
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, str, torch.Tensor]:
        ...
        if not self.fine_tuning:
            ...
            # print("xXXXXXXXXXXXXXXXXXX", mel.shape)
        # Fine-tuning logic that uses pre-computed mel. Example: Using TTS model-generated mel as input
            ...
            if source_sampling_rate != self.sampling_rate:
                audio = librosa.resample(audio, source_sampling_rate, self.sampling_rate)

            audio = audio[: len(audio) - (len(audio) % 320)]
            # Cast ndarray to torch tensor
            # _, _, start, end = get_as_alignment(filename, lang_id = "fra")
            ...
            #     os.path.join(
            #         self.base_mels_path,
            #         "CV17-whi-" + os.path.splitext(os.path.split(filename)[-1])[0] + ".npy",
            #     )
            # )
            # mel = np.fromfile(os.path.join(self.base_mels_path, os.path.basename(filename).split(".")[0] + ".ppg"))
            mel_idx = f_read_raw_mat(
                os.path.join(
                    self.base_mels_path, os.path.basename(filename).split(".")[0] + ".ppg"
                ),
                8,
            )
            pitch = f_read_raw_mat(
                os.path.join(
                    self.base_mels_path, "../F0/", os.path.basename(filename).split(".")[0] + ".f0"
                ),
                1,
            )
            pitch = (pitch - 61.64) / 67.5
            xvector = f_read_raw_mat(
                os.path.join(
                    self.base_mels_path,
                    "../Xvector/",
                    os.path.basename(filename).split(".")[0] + ".xvector",
                ),
                1,
            )
            xvector = (xvector) / 20

            x_old = np.linspace(0, 1, pitch.shape[0])  # Original x-values
            x_new = np.linspace(0, 1, mel_idx.shape[0])  # New x-values to interpolate to
            pitch = np.interp(x_new, x_old, pitch)  # Interpolated vector
            pitch = pitch.reshape(-1, 1)

            xvector = np.tile(xvector, (mel_idx.shape[0], 1))  # Use np.tile to repeat the vector

            mel = np.zeros((mel_idx.shape[0], 1280))

            for i in range(8):  # Since there are 8 quantizers
                # Extract the corresponding codebook
                codebook = self.codebooks[i]  # Shape (1024, 1280)

                # Gather the vectors from the codebook using the indices from mel_idx
                selected_vectors = codebook[mel_idx[:, i].astype(np.int64)]  # Shape (t, 1280)
                # Add the selected vectors to the mel tensor
                mel += selected_vectors

            # mel = upsample_time_axis(mel)
            # Create new x-axis for interpolation

            # print(mel.shape, pitch.shape, xvector.shape)
            mel = np.hstack((mel, pitch, xvector))
            # print(mel.shape, pitch.shape, xvector.shape)
            # mozz

            mel = torch.from_numpy(mel).float().transpose(0, 1)

            # print("XXXXXXXXXXXXXXXXXXXX", mel.shape, mel2.shape)
